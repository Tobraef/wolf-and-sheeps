use iced::{canvas::{Program, Frame, Path, event::Status}, Color, Point, Size, Rectangle};
use crate::game::{Board, Coord, physics::*, Species};
...
impl BoardGraphic {
    pub fn new(board: Board) -> Self { Self { board } }
    ...
    fn handle_mouse(&self, bounds: &Rectangle, mouse: Point) -> (Status, Option<GraphicMsg>) {
        ...
        if let Some(selected) = colliding {
            if current_mover_matches_selected(&self.board, selected) {
                return (Status::Captured, Some(GraphicMsg::PinSelected(selected.clone())));
            }
        }
...
fn pawns(frame: &mut Frame, block: f32, board: &Board) {
    ...
    if let Some(selected) = &board.selected {
        ...
        put_circle_in(frame, selected, SELECTED_RADIUS_RATIO, block, &selected_color);
    }
...
impl Program<GraphicMsg> for BoardGraphic {
    fn draw(
        ...
        _cursor: iced::canvas::Cursor) -> Vec<iced::canvas::Geometry> {
